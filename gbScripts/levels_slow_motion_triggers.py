...
def main(argv):
	...
	if level_index != 0:
		...
		root = data.getroot()
		route_objectgroup_node = {}
		for objectgroup in root:

			if objectgroup.tag == "objectgroup" and objectgroup.attrib["name"] == "route":
			
				route_objectgroup_node = objectgroup
				for route in objectgroup.findall('object'):

					objectgroup.remove(route)

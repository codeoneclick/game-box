...
namespace gb
{
    ...
    void mesh_3d_commiter_gbmesh3d::commit(const resource_transfering_data_shared_ptr& transfering_data)
    {
        ...
        std::shared_ptr<vbo::vertex_declaration_PT4B> vertex_declaration = std::make_shared<vbo::vertex_declaration_PT4B>(mesh->get_num_raw_vertices());
        ...
        for(ui32 i = 0; i < mesh->get_num_raw_vertices(); ++i)
        {
            ...
            vertices[i].m_bone_weights = glm::u8vec4(mesh->get_raw_vertices()[i].m_bones.size() >= 1 ? static_cast<ui8>(mesh->get_raw_vertices()[i].m_bones[0].m_weigth * 255.f) : 0,
            ...
            vertices[i].m_bone_ids = glm::u8vec4(mesh->get_raw_vertices()[i].m_bones.size() >= 1 ? static_cast<ui8>(mesh->get_raw_vertices()[i].m_bones[0].m_id) : 0,

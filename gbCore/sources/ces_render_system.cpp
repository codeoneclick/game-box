...
namespace gb
    ...
    void ces_render_system::on_feed_start(f32 dt)
    {
        ...
        if (m_visible_2d_entities.size() < render_pipeline::get_render_techniques_count())
        {
            m_visible_2d_entities.resize(render_pipeline::get_render_techniques_count());
        }
        
        if (m_visible_3d_entities.size() < render_pipeline::get_render_techniques_count())
        {
            m_visible_3d_entities.resize(render_pipeline::get_render_techniques_count());
        }
    }
    ...
    {
        const auto technique_index = ces_material_component::get_technique_name_index(technique_name);
        m_visible_2d_entities[technique_index].clear();
        ...
        ces_base_system::enumerate_entities_with_components(m_render_components_mask, [=](const ces_entity_shared_ptr& entity) {
            ...
            if(is_visible)
            {
                const auto material_component = entity->get_component_raw<ces_material_component>();
                const auto geometry_component = entity->get_component_raw<ces_geometry_component>();
                
                const auto& material = material_component->get_material(technique_index);
                const auto& mesh = geometry_component->get_mesh();
                
                if(material && material->get_shader()->is_commited() && mesh)
                {
                    const auto transformation_component = entity->get_component_raw<ces_transformation_component>();
                    ...
                    else if(transformation_component->is_3d())
                    {
                        if (transformation_component->is_in_camera_space())
                        {
                            ...
                            if (bounding_mode == ces_geometry_component::e_box)
                            {
                                ...
                                {
                                    is_visible = true;
                                    const auto camera_collision_component = entity->get_component_raw<ces_camera_collision_component>();
                                    if (camera_collision_component)
                                    {
                                        static glm::vec3 absolute_min_bound = glm::vec3(std::numeric_limits<f32>::max());
                                        static glm::vec3 absolute_max_bound = glm::vec3(std::numeric_limits<f32>::min());
                                        glm::transform(min_bound, mat_m, &absolute_min_bound);
                                        glm::transform(max_bound, mat_m, &absolute_max_bound);
                    ...
                    if(is_visible)
                    {
                        if(transformation_component->is_2d())
                        {
                            ...
                            m_visible_2d_entities[technique_index][z_order] = entity;
                        }
                        else if(transformation_component->is_3d())
                        {
                            if (draw_3d_entity_func)
                            {
                                draw_3d_entity_func(technique_name,
                                                    ...
                                                    entity->get_component_raw<ces_shader_uniforms_component>());
                            }
                            else
                            {
                                m_visible_3d_entities[technique_index].push(entity);
                            }
                        }
                    }
                }
    ...
    void ces_render_system::draw_2d_entities(const std::string &technique_name)
    {
        ...
        for (const auto& entity_it : m_visible_2d_entities[technique_index])
        {
            ...
            if(!entity_weak.expired())
            {
                ...
                auto material = material_component->get_material(technique_index);
    ...
    void ces_render_system::draw_3d_entities(const std::string &technique_name)
    {
        ...
        while(!m_visible_3d_entities[technique_index].empty())
        {
            auto entity_weak = m_visible_3d_entities[technique_index].front();
            if(!entity_weak.expired())
            {
                const auto& entity = entity_weak.lock();
                const auto geometry_component = entity->get_component_raw<ces_geometry_component>();
                const auto transformation_component = entity->get_component_raw<ces_transformation_3d_component>();
                const auto material_component = entity->get_component_raw<ces_material_component>();
                const auto shader_uniforms_component = entity->get_component_raw<ces_shader_uniforms_component>();
                draw_3d_entity(technique_name, technique_index, material_component, geometry_component, transformation_component, shader_uniforms_component);
            }
            m_visible_3d_entities[technique_index].pop();
        }
    }
                                           ...
                                           ces_shader_uniforms_component* shader_uniforms_component)
    {
        ...
        if(material->get_is_batching())
        {
            m_batching_pipeline->batch(material, mesh->as_3d(), mat_m, transformation_component->get_matrix_m_version());
        }
        else

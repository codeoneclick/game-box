...
namespace gb
{
	...
	LRESULT CALLBACK win32_callback(HWND hwnd, UINT message, WPARAM param_w, LPARAM param_l)
	{
		...
		case WM_LBUTTONDOWN:
		{
			input_context->gr_pressed(current_touch_point, gb::e_input_source_mouse_left);
		...
		case WM_LBUTTONUP:
		{
			input_context->gr_released(current_touch_point, gb::e_input_source_mouse_left);
			input_context->set_previous_touch_point(current_touch_point);
		...
		if (input_context->m_is_tapped)
		{
			...
			input_context->gr_dragged(current_touch_point, delta, gb::e_input_source_mouse_left);
			input_context->set_previous_touch_point(current_touch_point);
		}

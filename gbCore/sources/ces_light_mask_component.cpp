...
namespace gb
{
    ...
    bool ces_light_mask_component::is_need_to_recalculate_shadowcasters_geometry() const
    {
        return !m_is_shadow_geometry_calculated;
    }
    ...
    void ces_light_mask_component::push_shadowcasters_geometry(const std::vector<std::vector<glm::vec2>>& shadowcasters_geometry)
    {
        ...
        m_shadow_geometry_to_apply = std::make_tuple(m_center, m_bounds_geometry, shadowcasters_geometry);
        m_is_shadow_geometry_calculated = false;
    }
    ...
    void ces_light_mask_component::apply_shadowcasters_geometry()
    {
        ...
        {
            std::lock_guard<std::mutex> guard(m_shadow_geometry_apply_mutex);
            center = std::get<0>(m_shadow_geometry_to_apply);
            bounds = std::get<1>(m_shadow_geometry_to_apply);
            shadowcasters_geometry = std::get<2>(m_shadow_geometry_to_apply);
        }
        ...
        {
            {
                ...
                m_transfer_vertices_count = vertices_count;
                m_transfer_indices_count = indices_count;
                memcpy(m_transfer_vertices, m_working_vertices, m_transfer_vertices_count * sizeof(vbo::vertex_attribute_PTC));
                memcpy(m_transfer_indices, m_working_indices, m_transfer_indices_count * sizeof(ui16));
            }
            ...
            operation->set_execution_callback([this]() {
                vbo::vertex_attribute_PTC* vertices = m_mesh->get_vbo()->lock<vbo::vertex_attribute_PTC>();
                ui16* indices = m_mesh->get_ibo()->lock();
                {
                    std::lock_guard<std::mutex> guard(m_transfer_to_vram_mutex);
                    memcpy(vertices, m_transfer_vertices, m_transfer_vertices_count * sizeof(vbo::vertex_attribute_PTC));
                    memcpy(indices, m_transfer_indices, m_transfer_indices_count * sizeof(ui16));
                }
                m_mesh->get_vbo()->unlock(m_transfer_vertices_count);
                m_mesh->get_ibo()->unlock(m_transfer_indices_count);
            });
            operation->add_to_execution_queue();
        }
        m_is_shadow_geometry_calculated = true;
    }

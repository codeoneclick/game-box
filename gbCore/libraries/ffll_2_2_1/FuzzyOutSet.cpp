...
FuzzyOutSet::FuzzyOutSet(FuzzyVariableBase* par) : FuzzySetBase(par), FFLLBase(par) 
{
    ...
    FuzzyModelBase* parent = par->get_parent();
    const auto deffuz_method = parent->get_defuzz_method();
	set_defuzz_method(deffuz_method);
...
int FuzzyOutSet::set_defuzz_method(int type)
{
	...
	switch(type)
		...
		case DefuzzVarObj::DEFUZZ_COG:
			...
			static_cast<COGDefuzzSetObj*>(defuzz_obj)->init(FuzzyVariableBase::get_dom_array_count());
...
RealType FuzzyOutSet::get_defuzz_x(int dom /* = -1 */)
{
	...
	if (defuzz_type == DefuzzVarObj::DEFUZZ_COG)
		{
		COGDefuzzSetObj* cog_defuzz = static_cast<COGDefuzzSetObj*>(defuzz_base);
	...
	if (defuzz_type == DefuzzVarObj::DEFUZZ_MOM)
		{
		MOMDefuzzSetObj* mom_defuzz = static_cast<MOMDefuzzSetObj*>(defuzz_base);
...
{
    FuzzyVariableBase* parent = FuzzySetBase::get_parent();
	return (FuzzyOutVariable*)(parent);
};

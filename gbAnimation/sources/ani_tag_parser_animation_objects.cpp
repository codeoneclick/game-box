...
namespace gb
{
    namespace anim
    {
        ...
        void ani_tag_parser_animation_objects::read(const ani_parser_t& parser,
                                                    ...
                                                    const std::shared_ptr<ani_timeline>& timeline)
        {
            ...
            for (ui32 i = 0; i < count; ++i)
            {
                ...
                if (stream->get_input()->get_header()->get_major_version() >= 4)
                {
                    ...
                    timeline->push_animation_object(object_id, element_atlas_id_reference, static_cast<e_ani_character_type>(character_type));
                }
                else
                {
                    timeline->push_animation_object(object_id, element_atlas_id_reference, e_ani_character_type::e_texture);
                }
            }
        }
    }

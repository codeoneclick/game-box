...
namespace gb
{
    namespace anim
    {
        ...
        void ani_tag_parser_timeline::read(const ani_parser_t& parser,
                                           ...
                                           const std::shared_ptr<ani_timeline>& timeline)
        {
            ...
            i8 has_linkage = stream->read_ubyte();
            if (has_linkage)
            {
                std::string linkage_name;
                stream->read_string(&linkage_name);
                new_timeline->set_linkage_name(linkage_name);
            }

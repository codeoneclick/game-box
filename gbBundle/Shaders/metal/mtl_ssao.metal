                                       ...
                                       texture2d<half> random_texture [[texture(2)]])
{
    ...
    const half3 normal = normal_texture.sample(linear_sampler, in.texcoord).xyz;
    ...
    for (int i = 0; i < iterations; ++i)
    {
        ...
        half intensity = saturate((dot(ao_direction / ao_distance, normal) - bias_value) * (1.f / (1.f + ao_distance)));
        ...
        intensity = saturate((dot(ao_direction / ao_distance, normal) - bias_value) * (1.f / (1.f + ao_distance)));
